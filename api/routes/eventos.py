...
from config.cloudnary import upload_image
...
async def crear_evento(nuevoEvento: evento, data: tuple = Depends(verificar_centro)):
    ...
    base64_str = nuevoEvento.base64_str
    url=""
    public_id=None

    if base64_str is not None:
        url, public_id = upload_image(base64_str)
    else:
        url="https://placehold.co/400x200/FF6B1A/white?text=Adda"

    
    nuevo_evento = {"titulo": nuevoEvento.titulo, "ubicacion": nuevoEvento.ubicacion, "fecha":nuevoEvento.fecha,"categoria":nuevoEvento.categoria,"imagen_url":url, "descripcion": nuevoEvento.descripcion, "centro_uid": centro_uid, "estatus":"activo", "public_id":public_id}
...
async def actualizar_evento(id:str,actualEvento: evento, data: tuple = Depends(verificar_centro)):
    ...
    doc = doc_ref.get()
    if doc.exists:
        doc_ref.update({"titulo": actualEvento.titulo, "ubicacion": actualEvento.ubicacion,"categoria":actualEvento.categoria, "descripcion": actualEvento.descripcion})
        if actualEvento.fecha:
            doc_ref.update({"fecha": actualEvento.fecha})
        if actualEvento.base64_str:
            public_id = doc.to_dict()["public_id"]
            url, public_id_return = upload_image(actualEvento.base64_str, public_id)
            doc_ref.update({"imagen_url": url})
